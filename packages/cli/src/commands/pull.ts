...
export async function pullCommand(options: PullOptions): Promise<void> {
  ...
  if (!repoSlug) {
    error('No repo specified. Pass --repo owner/name or run dotenvy init first.')
    process.exit(1)
  }
  if (!envName) {
    error('No environment specified. Pass --env <name> or run dotenvy init first.')
    process.exit(1)
  }
  ...
  if (missingRequired.length > 0) {
    ...
    hint('Set these values in the Dotenvy web UI, then run dotenvy pull again.')
    process.exitCode = 1
  }
}
...
function buildEnvFile(
  ...
  const lines: string[] = [
    `# Generated by dotenvy — ${meta.repoFullName} / ${meta.environment}`,
  ...
  if (localOnlyEntries.length > 0) {
    lines.push('')
    lines.push('# ─── Local only (not managed by Dotenvy) ────────────────────────────────────')
...
function runCheck(
  ...
  if (existingVars === null) {
    ...
    hint(`Run ${fmt.bold('dotenvy pull')} to create it.`)
    return false
  }
  ...
  blank()
  hint(`Run ${fmt.bold('dotenvy pull --force')} to update.`)
  return false
}
...
function printConflictPreview(
  ...
  hint(`  ${fmt.bold('dotenvy pull --force')}     apply changes (merge, preserves local-only vars)`)
  hint(`  ${fmt.bold('dotenvy pull --dry-run')}   preview without writing`)
}
...
async function resolveIds(
  ...
  try {
    ...
    if (!repo) {
      spin.stop()
      error(`Repo "${repoSlug}" not found in Dotenvy. An admin needs to register it first.`)
      process.exit(1)
    }
    ...
    if (!env) {
      ...
      hint(`Run ${fmt.bold('dotenvy envs --repo ' + repoSlug)} to see available environments.`)
      process.exit(1)
    }
